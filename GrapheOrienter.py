...
class GrapheOriente(Tk):
  ...
  def __init__(self):
    ...
    filemenu = Menu(menubar, tearoff = 0)
    filemenu.add_separator()
    # filemenu.add_command(label = "Graphe profond ", command=self.dfs)
    ...
    filemenu = Menu(menubar, tearoff = 0)
    filemenu.add_separator()
    # menubar.add_command(label = "Supprimer un point", command =self.deletePoint)
  ...
  def deletePoint(self, event=None):
    ...
    self.label=tk.Label(self.toplevel_dialog, text='Entrer le sommet : ')
    ...
    self.yes_button=ttk.Button(self.toplevel_dialog,text='Valider',width=25,command=self.Close_del)
    self.yes_button.pack(side='right',fill='x',expand=True)
      
  def Close_del (self,event=None):
    #element = self.temp[0][0]
    #self.point.append([event.x,event.y,self.sommet,self.numero,self.i])
    i = self.toplevel_dialog
    #self.point.remove([self.i])
    #self.sommets.remove(self.i)
    self.graphe.delete(self.sommet, self.numero, self.i)
    #self.i-=1
...
"""
  ...
  def Close_arc (self, event=None):
    ...
    if self.temp[0][0] < self.temp[1][0]:
      ...
      #distancex = self.valeur.get()
     # self.sommet=self.graphe.create_oval(self.temp[0][0]-20,self.temp[0][1]-20,self.temp[0][0]+10,self.temp[0][1]+10, fill="#fff")
      #self.numero=self.graphe.create_text((self.temp[0][0],self.temp[0][1]),(self.temp[1][0],self.temp[1][1]),text="{}".format(distancex))
      ...
      #distancex = self.valeur.get()
      #self.sommet=self.graphe.create_oval(self.temp[0][0]-10,self.temp[0][1]-25,self.temp[0][0]+1,self.temp[0][1], fill="#000")
      #self.numero=self.graphe.create_text(self.temp[0][0],self.temp[0][1],text="{}".format(distancex))
